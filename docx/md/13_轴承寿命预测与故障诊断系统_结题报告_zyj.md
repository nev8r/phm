...
PHM2012 RUL 主线使用 Hann window 后的 rFFT 幅值，移除 DC 分量，取前 256 个 `log1p` 频域 bin，并补充 RMS、峭度、谱心、频带能量等 20 维退化统计特征，按 32 个快照组成序列，模型输入形状为 `[B, 32, 276]`。训练流程采用 MPS 设备，结果记录为 validation MSE、test MSE、test RMSE 和 test MAE。

XJTU-SY 故障诊断主线使用二分类模型，输入为 8 个快照窗口，每个快照 552 维特征：双通道各包含 FFT 256 维、8 个时域特征、7 个频域统计特征和 5 个频带能量特征。测试集结果记录 accuracy、macro-F1、fault-F1 和混淆矩阵。
...
| RUL 预测 | 已完成主线 | PHM2012 指标图与 Notebook 输出 |
