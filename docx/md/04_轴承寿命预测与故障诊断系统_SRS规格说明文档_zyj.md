...
| 编号 | 需求 | 优先级 | 验收方式 |
|---|---|---|---|
| FR-01 | 读取 PHM2012 与 XJTU-SY 数据集 | 高 | Loader 测试与 CLI 运行 |
| FR-02 | 生成 FFT、时域、频域和频带能量特征 | 高 | 特征处理单元测试 |
| FR-03 | 构造 RUL 标签与健康/故障标签 | 高 | 数据集样本 shape 与标签检查 |
| FR-04 | 训练 CBAM-CNN-LSTM RUL 模型 | 高 | 输出 MSE、RMSE、MAE、R2 |
| FR-05 | 训练 ResCNN-LSTM 故障诊断模型 | 高 | 输出 accuracy、F1、混淆矩阵 |
| FR-06 | 支持 CLI 可视化展示 | 中 | 生成特征图、曲线图、热力图 |
...
| 需求 | 设计/实现位置 | 测试证据 |
|---|---|---|
| FR-01/FR-02 | `phm.data`、`phm.data.process` | `tests/test_feature_processors.py`、CLI smoke |
| FR-04 | `phm.model.paper.PaperCBAMCNNLSTMRegressor` | PHM2012 RUL CLI |
| FR-05 | `phm.model.paper.ResCNNLSTMClassifier` | XJTU-SY Fault CLI |
| FR-06 | `examples/3-papers` | `docx/img` 图表 |
