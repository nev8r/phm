...
PHM2012 RUL 使用 Hann window、rFFT 幅值、去 DC、`log1p` 缩放和训练集标准化，适配论文主线的频域退化建模。XJTU-SY 故障诊断使用双通道特征组合：FFT 频谱描述冲击频率，时域统计描述能量与尖峰，频域统计描述谱心、带宽和熵，频带能量描述不同频段能量占比。
...
| RUL 模型 | CBAM-CNN-LSTM | 结合卷积、注意力和 LSTM，贴合 PHM2012 论文主线 |
| 诊断模型 | ResCNN-LSTM | 残差卷积提取局部模式，LSTM 聚合时间窗口 |
