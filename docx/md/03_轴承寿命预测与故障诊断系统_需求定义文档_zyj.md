...
本系统面向课程答辩、代码评审和本地实验验证三类使用场景。课程评审关注需求、设计、测试和交付完整性；开发成员关注数据路径、训练流程和结果检查；演示使用者关注 Notebook 中能否从真实数据得到图表和指标。
...
本项目不接入实时采集硬件，不提供云服务和多用户权限系统，不承诺工业现场部署。所有实验结论均以公开数据集、本地配置和课程实验为边界。
