...
- focused 测试：`uv run --extra dev pytest tests/test_rul_metrics.py tests/test_paper_cnn_lstm_attention.py tests/test_paper_xlstm_transformer.py tests/test_examples_notebooks.py -q`，结果为 `20 passed in 6.40s`。
- 全量测试：`uv run --extra dev pytest -q`，结果为 `31 passed in 8.45s`。
