...
各位老师、同学好，我们小组汇报的题目是“工业轴承故障预测系统的实现”。这里的故障预测不是故障类型分类，而是面向预测性维护的剩余寿命预测，也就是 RUL 预测。
...
所以项目主线是 RUL 回归。XJTU-SY 和 PHM2012 都是 run-to-failure 数据，也就是从正常运行持续采到失效附近的退化过程。这样的数据更适合做剩余寿命预测、健康指标和生存概率分析，而不是简单故障分类。
...
这一页正式汇报快速带过即可。如果老师问为什么不是分类，我们回答：数据是退化序列，主任务是预测还能运行多久。
