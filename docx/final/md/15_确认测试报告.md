...
课程文档交付要求：系统通过 `scripts/export_course_docs.sh` 导出 Markdown 对应 PDF 和 DOCX，最终保留 22 份 Markdown、22 份 PDF、22 份 DOCX。
...
| 实验 | comparison 文件 |
| --- | --- |
| CNN-LSTM-AM | `tmp/paper_repro_real_metrics/paper_cnn_lstm_attention/comparison_metrics.csv` |
| xLSTM-Transformer | `tmp/paper_repro_xlstm_transformer/paper_xlstm_transformer/comparison_metrics.csv` |
...
2026-06-14 结题验收中，focused 测试结果为 `20 passed in 6.40s`，全量测试结果为 `31 passed in 8.45s`。两篇论文复现均完成 8 epoch 真实数据训练并输出指标表。
