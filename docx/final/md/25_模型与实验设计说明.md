...
完整 19 维字段如下。这里的“14 个时域特征 + 5 个频域特征”是项目主线自己用 NumPy/FFT 实现的默认特征集合，不是调用 `tsfresh` 自动生成特征。#2 后已新增 `tsfresh` train-only 特征分析与 `sktime` baseline；它们用于指标驱动补充实验，不替代本节论文复现主输入。
