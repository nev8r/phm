...
其中 19 维特征是 `SignalFeatureExtractor` 中的自定义 NumPy/FFT 实现，不是 `tsfresh` 自动特征；结题阶段已补充 `tsfresh` Minimal/Efficient train-only 自动特征分析、held-out RUL baseline 和 manual+tsfresh 融合输入验证，当前结论是相关性整体偏弱，定位为自动特征分析旁证。
