# 指标驱动实验结果说明与 RUL 改进任务书

本文档把 `next goal.md` 整理为后续开发任务书。它不是本轮新增代码实现说明，而是下一阶段从“系统架构完成度”转向“RUL 预测指标提升”的设计建议、实验方案、验收标准和风险边界。

## 1. 目标与非目标

下一阶段目标是建立统一实验协议，在相同 train/test 轴承划分、相同 RUL 标签、相同评价指标下，比较手工 19 维特征、tsfresh 自动特征、sktime 时间序列回归 baseline 与现有深度模型，找出指标更优的特征和模型组合。

本轮不在本地实现 tsfresh、sktime 或新模型代码，不提交新的实验结果 CSV，也不声称新增方法已经优于现有论文复现模型。本轮交付的是可执行的后续任务书，以及后续 `08_指标驱动实验结果说明` 应如何填写的结构化模板。

当前应保留的事实边界如下：

| 项目 | 当前事实 | 后续改进方向 |
| --- | --- | --- |
| 数据链路 | loader、特征、标签、训练、评价已形成闭环 | 统一为可插拔实验平台 |
| 主线特征 | 19 维手工时频域特征 | 补充 tsfresh 自动特征分析 |
| 主线模型 | CNN-LSTM-AM、xLSTM-Transformer | 增加 sklearn/sktime baseline |
| 真实复现 | 每轴承 96 快照抽样，50 epoch | 扩到 192/384 快照与多随机种子 |
| 评价指标 | RMSE、NormalizedRMSE、R2、Score | 用统一指标表排名 |
