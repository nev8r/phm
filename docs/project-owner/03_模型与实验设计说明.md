...
完整 19 维字段如下。这里的“14 个时域特征 + 5 个频域特征”是项目主线自己用 NumPy/FFT 实现的默认特征集合，不是调用 `tsfresh` 自动生成特征。结题阶段已把 `tsfresh` 作为补充实验评估：Minimal/Efficient 配置均采用 train-only 特征选择，held-out Bearing1_3 只用于最终评估；结果用于说明自动特征候选和 manual+tsfresh 融合边界，不替代本轮论文复现的主输入来源。
