...
本项目的主线是 **RUL 预测和轴承健康趋势分析**。两个数据集都更适合做剩余寿命预测、健康指标构造和退化阶段标注；失效概率建模与生存分析作为预测性维护扩展方向保留接口和数据说明，不应把它们简单理解为失效类型分类数据集。
...
- 带温度通道的多源特征分析；
- 同工况内少样本训练和测试；
- 后续使用 `C-index`、Brier Score 等生存分析指标做扩展评估。

它不适合直接做失效部位分类，因为 challenge 设定中并没有给出每条样本的明确失效类型标签。
...
| 生存分析扩展 | 使用多个 bearing 的 `duration`、`event` 和特征表训练 |
| 报告展示 | 展示 `rul` 曲线、退化趋势曲线、预测误差和核心指标 |
...
| --- | --- | --- |
| 主要用途 | 轴承全寿命退化与 RUL 预测 | PHM challenge RUL 预测，兼具生存分析扩展价值 |
| 工况数 | 3 | 3 |
...
### 6.6 两个数据集是否能用于失效类型分类？

本项目不把这两个数据集作为失效类型分类数据集使用。它们在本项目中的定位是 RUL 预测、退化阶段分析和预测性维护扩展研究。若未来要做失效类型分类，需要额外引入明确标注失效类型的数据集或标签规则。
