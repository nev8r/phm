...
本项目的主线是 **RUL 预测、失效概率建模与生存分析**。两个数据集都更适合做剩余寿命预测、健康指标构造、退化阶段划分和生存概率分析，不应把它们简单理解为失效类型分类数据集。
...
本项目不把这两个数据集作为失效类型分类数据集使用。它们在本项目中的定位是 RUL 预测、退化阶段分析、失效概率建模和生存分析。若未来要做失效类型分类，需要额外引入明确标注失效类型的数据集或标签规则。
