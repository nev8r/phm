...
本项目围绕轴承 PHM 做了两条主线：第一条是 PHM2012 的 RUL 预测，用来回答轴承还能运行多久；第二条是 XJTU-SY 的故障检测，用来回答当前是否已经进入故障阶段。系统层面，我们完成了数据加载、特征工程、模型训练、指标评估、Notebook 可视化和课程交付文档。实验层面，RUL 和 Fault 都能跑完整流程，并输出图表和指标。
...
**5. 系统和零散实验脚本有什么区别？**  
答：系统把数据加载、特征工程、模型训练、评估输出和可视化组织成统一流程，训练走 `BaseTrainer`，结果有 JSON、CSV、图表、测试和文档支撑，不只是单次实验能跑。
