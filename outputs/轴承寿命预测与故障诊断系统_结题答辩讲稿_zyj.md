...
> 建议节奏：13 页，约 8-10 分钟。讲法按“为什么做、系统是什么、数据怎么来、模型为什么这样选、训练跑出了什么、谁做了什么”展开。
...
RUL 这边使用 CBAM-CNN-LSTM。CNN 负责从频域和退化统计特征里提取局部模式，CBAM 负责对更关键的通道和时间位置加权，LSTM 负责建模 32 个连续快照之间的退化依赖。我们没有上更重的 Transformer，主要是考虑课程算力和复现成本；当前数据规模下，CNN-LSTM 结构更轻，训练速度更可控，也更容易解释。

Fault 这边使用 ResCNN-LSTM。残差卷积可以稳定提取局部特征，LSTM 聚合 8 个快照窗口，最后输出 Healthy/Faulty 二分类结果。这个任务的标签和 RMS、能量变化相关性比较强，所以 ResCNN-LSTM 在性能、速度和答辩可解释性之间比较平衡。

![RUL 模型架构](../docx/img/rul_model_architecture.png)

![Fault 模型架构](../docx/img/fault_model_architecture.png)

## 第 7 页：训练例子一 PHM2012 RUL
...
本次全量训练使用 `BaseTrainer` 跑完整流程，训练轮数设置为 200，设备使用 Mac 上的 MPS。训练过程中保存最佳验证点、checkpoint、标准化参数、预测 CSV 和可视化图。最终最佳验证点在 epoch 194，validation MSE 为 0.000098；论文主线测试轴承上的 test MSE 为 0.021532，RMSE 为 0.1467，MAE 为 0.1050。
...
| 测试窗口 | 10995 | 6 个论文主线测试轴承 |
| 最大训练轮数 | 200 | 使用 `BaseTrainer` 完整训练 |
| 最佳 epoch | 194 | 按 validation MSE 选择 |
| Validation MSE / RMSE / MAE | 0.000098 / 0.0099 / 0.0077 | 验证集拟合较好 |
| Test MSE / RMSE / MAE | 0.021532 / 0.1467 / 0.1050 | 主线测试集指标 |
| 主要输出目录 | `outputs/runs/20260618_143026_train_rul` | 保存配置、指标、预测 CSV 和图表 |

![PHM2012 RUL 训练与验证曲线](../docx/img/training_validation.png)

讲第一张图的时候可以说：训练曲线能看到模型在前期快速下降，后期趋于稳定；验证曲线最低点出现在 epoch 194，所以最终选择这一轮作为最佳模型。

![PHM2012 RUL 预测曲线](../docx/img/phm2012_rul_prediction_curves.png)

讲第二张图的时候可以说：RUL 预测曲线整体跟随真实寿命下降趋势，增强训练后拟合比之前明显正常。但不同轴承上的拟合程度仍有差异，这说明如果要追论文最优，还需要继续做跨轴承泛化、随机种子和更严格的数据划分对齐。

## 第 8 页：训练例子二 XJTU-SY Fault
...
这条主线的测试结果比较高：Accuracy 为 0.9971，Macro-F1 为 0.9959。混淆矩阵中，Healthy 样本有 1044 个判断正确、2 个误判为 Faulty；Faulty 样本有 318 个判断正确、2 个误判为 Healthy。
...
| Test loss | 0.0117 | 测试集分类损失 |
| Accuracy | 0.9971 | 整体分类正确率 |
| Macro-F1 / Weighted-F1 / Fault-F1 | 0.9959 / 0.9971 / 0.9938 | 兼顾类别均衡和故障类表现 |
| 混淆矩阵 | `[[1044, 2], [2, 318]]` | 行是真实类别，列是预测类别 |
| 论文对比 | Paper Acc/F1 0.9639/0.9640；Ours Acc/F1 0.9971/0.9971 | 当前二分类主线达到可复现展示效果 |

![XJTU-SY Fault 混淆矩阵](../docx/img/xjtu_confusion_matrix.png)

讲图的时候可以说：混淆矩阵左上和右下的数量占绝大多数，说明 Healthy 和 Faulty 两类边界比较清楚；误判一共只有 4 个样本。
...
这一页说明我们不是只跑深度模型，也补了统一 baseline。

RUL 用同一 split、同一标准化策略和同一缓存特征，对比 Ridge、RandomForest、RocketRegressor 和 CBAM-CNN-LSTM。结果里 CBAM-CNN-LSTM 的 test RMSE 是 0.1467，明显低于 Ridge 的 0.5367、RandomForest 的 0.2762 和 RocketRegressor 的 0.3040，说明对 RUL 这种退化序列，序列深度模型确实更适合。

Fault 这边 LogisticRegression 的 Weighted-F1 也是 0.9971，ResCNN-LSTM 也是 0.9971。这说明当前 Healthy/Faulty 二分类边界非常清楚，传统线性模型也能拿到很高结果。答辩时这里要讲得克制：这不是说明深度模型一定碾压，而是说明当前任务和特征已经具备很强可分性。

![RUL Baseline 对比](../docx/img/rul_benchmark.png)

![Fault Baseline 对比](../docx/img/fault_benchmark.png)

## 第 10 页：统一 CLI 与任务关系

这一页讲工程闭环怎么跑。

系统提供统一命令入口，分析、训练、benchmark 和报告都可以从命令行启动。比如：
...
每次运行都会输出到 `outputs/runs/<timestamp>_<task>/`，里面有 `config.json`、`metrics.json`、`predictions.csv`、图表和模型摘要。这样老师如果要追溯结果，可以直接看 run 目录里的配置、指标和预测文件。

RUL 和 Fault 的关系也可以在这一页讲清楚：它们共享同一个数学流程，都是原始振动信号经过特征映射变成序列样本，再输入模型。不同的是，RUL 输出连续寿命值，Fault 根据 RMS 3σ 首次故障点 FOT 把状态离散成 Healthy 和 Faulty。
...
| 邹扬 | 25% | CLI 工程入口、工具函数、可视化图表、测试用例、实验结果整理和交付检查 |
...
**3. PHM2012 特征为什么这样设计？**
答：PHM2012 是轴承从健康到失效的振动退化数据，RUL 预测既要看频谱形态，也要看退化强度和时间趋势。Hann window 是为了减少短时采样边界带来的频谱泄漏；rFFT 能把振动信号转成频域模式，便于 CNN 提取局部频谱变化；取前 256 个频域 bin 是信息量和算力成本的折中，避免全频输入过高维、噪声过多；`log1p` 用来压缩频谱幅值动态范围，让训练更稳定。额外加入 RMS、峭度、谱心、频带能量等 20 维手工特征，是为了补充可解释的退化强度信息。最后用 32 个连续快照组成序列，是因为 RUL 不是单点判断，而是退化趋势预测，需要让 LSTM 看到寿命随时间下降的过程。

可以简化成一句话：FFT 给模型看频谱形态，手工特征给模型看退化强度，32 步序列给模型看退化趋势。
