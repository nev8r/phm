...
\midrule
\endhead
\code{monotonicity} & 单调性。看同一轴承内特征是否主要朝一个方向变化。 \\
\code{robustness} & 鲁棒性。看特征曲线是否平滑、是否不容易被噪声支配。 \\
\code{trendability} & 趋势一致性。看不同轴承的特征曲线形状是否相似。 \\
\code{prognosability} & 末端一致性。看不同轴承临近失效时的特征值是否集中。 \\
\bottomrule
\end{longtable}
