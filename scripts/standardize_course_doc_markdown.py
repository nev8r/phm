...
REPLACEMENTS = {
    ...
    "故" + "障分类仅保留基础能力，不作为结题主线": "离散状态划分仅作为退化阶段辅助，不作为结题主线",
    "分类不是主线": "离散诊断不是主线",
    "不扩展" + "故" + "障诊断分类指标": "不扩展离散状态识别指标",
    "不以" + "故" + "障分类作为结题重点": "不以离散状态识别作为结题重点",
    "故" + "障诊断或分类": "离散状态识别",
    "阶段分类流程": "退化阶段标注与可视化流程",
    "阶段分类数据集": "退化阶段标注数据集",
...
系统架构围绕 RUL 预测数据流组织，而不是围绕某一个模型组织。设计上分为“数据接入、特征标签、模型训练、评价展示”四个主层：
...
3. 业务主线统一表述为“预测性维护、轴承健康评估、剩余寿命预测、生存概率/失效风险分析”，不把项目主线写成离散状态识别。
...
正式汇报时建议保持课程项目口吻：多用“我们完成了、我们验证了、目前的边界是”，少用无法由当前实验直接证明的强比较或部署承诺。讲到论文复现时，要主动说明这是基于本项目特征管线的小规模真实训练复现，重点是结构、流程和指标体系可复跑。
""",
        ),
    ],
}
